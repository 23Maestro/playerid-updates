...
import { Form, ActionPanel, Action, showToast, Toast, LaunchProps, environment } from "@raycast/api";
import { useForm, FormValidation } from "@raycast/utils";
import { exec } from "child_process";
import { promisify } from "util";
import path from "path"; // Needed if using relative paths, but we're using absolute
  ...
  const { handleSubmit, itemProps, reset } = useForm<EmailFormValues>({
    async onSubmit(formValues) {
      ...
      try {
        ...
        // Using absolute path as per recent discussions for other scripts
        const scriptPath = "/Users/singleton23/Desktop/RAYCAST/playerid-updates/email_automation.py";
